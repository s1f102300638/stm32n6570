...
{
	/* UART is already initialized by HAL in main.c */
	/* Just ensure it's enabled and ready */
	
	/* The HAL has already configured the UART, so we just need to ensure it's running */
	/* No need to reconfigure BRR as HAL_UART_Init has already done it */
	
	/* Optional: Force re-enable if needed */
	if ((UART_CR1 & CR1_UE) == 0) {
		/* If UART is not enabled, initialize it */
		UART_CR1 = 0;		/* 8bit, Non parity (Reset value) */
		UART_CR2 = 0;		/* Stop bit 1 (Reset value) */
		UART_CR3 = 0;		/* No hard flow control (Reset value) */
		
		/* Calculate baud rate register value
		 * For STM32N6 with current clock config:
		 * PLL1_P2 output = 600MHz / 1 = 600MHz
		 * IC2 = 600MHz / 3 = 200MHz (System clock)
		 * APB1 = 200MHz / 2 = 100MHz (from AHB DIV2)
		 * USART1 is on APB2 which is also 100MHz
		 * BRR = fck / baud rate = 100000000 / 115200 = 868 (0x364)
		 */
		UART_BRR = 0x0364;	/* Set baud rate for 115200bps @ 100MHz */
...
#endif /* MTKBSP_DISCOVERY_STM32N657 */
#endif /* USE_TMONITOR */
